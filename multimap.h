...
public:
    ...
    template<class InputIterator>
    multimap(InputIterator first , InputIterator last , Compare& comp) : t(comp){t.insert_equal(first , last);}
    ...
    void swap(const multimap<Key , T , Compare , Alloc>& x){t.swap(x.t);}
...
inline bool operator == (const multimap<Key , T , Compare , Alloc>& x , const multimap<Key , T , Compare , Alloc>& y){return x.t == y.t;}
